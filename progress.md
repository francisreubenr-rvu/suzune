...
- M2 IN PROGRESS: agents W3 (crates/cleanup — llama-server child process + OpenAI-compat client + prompt v3) and W4 (crates/inject — AxInsert -> ClipboardPaste fallback chain) running. First W3 attempt died on session limit before writing anything; relaunched after reset. After they land: main session verifies (incl. running crates/inject/examples/inject_demo with user at keyboard), writes coordinator + Tauri glue, commits.
