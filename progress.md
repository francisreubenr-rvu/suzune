...
- Settings file materializes at ~/Library/Application Support/dev.whispr.app/settings.json (models_root defaults to /Volumes/1TB SSD/LM/whispr-models on this machine).
...
- Session 2026-07-04 side-note: a SEPARATE Swift/WhisperKit whispr implementation exists at /Users/maverick/codex/whispr (different Claude session, bundle id com.whispr.Whispr). Do not confuse the two; watch for global-shortcut and TCC permission crossfire when both run.
...
- M4 DONE (55c151b): paperback app icon, README (measured claims only), Info.plist (mic usage + LSUIElement), whispr.app + dmg bundles, pushed PRIVATE to https://github.com/francisreubenr-rvu/whispr. AppleDouble files also corrupt .git/objects/pack — run find .git -name '._*' -delete if git errors appear.
