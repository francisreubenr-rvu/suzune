...
suzune = Handy's local-first architecture + the embedded local LLM cleanup layer Handy lacks + Wispr-grade formatting UX + a distinctive paperback visual identity. Free, open-source (MIT).
...
## 4. How suzune beats Wispr Flow (mapped to findings weakness table)

| Wispr weakness (rank) | suzune answer |
|---|---|
...
|---|---|---|
| D1 | Fork Handy vs greenfield | **Greenfield Tauri app, porting Handy's proven patterns with attribution in README/NOTICE.** Forking inherits 24k-star project's UI/identity and gaps; suzune's value is a distinct product. MIT permits pattern/code reuse with attribution. |
