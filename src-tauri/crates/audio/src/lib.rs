...
//! whispr's scope: no built-in VAD gating or spectrum visualizer in this
//! crate — those live in `whispr-vad` and the UI layer respectively.
...
fn preferred_input_config(device: &Device) -> Result<cpal::SupportedStreamConfig> {
    ...
    let supported = match device.supported_input_configs() {
        Ok(configs) => configs,
        Err(e) => {
            log::warn!("whispr-audio: could not enumerate input configs ({e}), using default");
...
fn build_input_stream(
    ...
    match config.sample_format() {
        ...
        other => {
            log::error!("whispr-audio: unsupported sample format {other:?}");
    ...
    f32: cpal::FromSample<T>,
{
    ...
    let callback = move |data: &[T], _: &cpal::InputCallbackInfo| {
        ...
        if raw_tx.send(RawChunk::Samples(mono)).is_err() {
            log::error!("whispr-audio: sample channel closed, dropping audio");
        }
    };
    ...
    device.build_input_stream(
        &config.clone().into(),
        callback,
        |err| log::error!("whispr-audio: stream error: {err}"),
        None,
    )
}
...
fn run_worker(
    ...
    let emit_frame = |frame: &[f32], recording: bool, utterance: &mut Vec<f32>| {
        ...
        if frame_tx.send(frame.to_vec()).is_err() {
            log::trace!("whispr-audio: no live-frame subscribers");
        }
    };
                ...
                Cmd::Stop(reply_tx) => {
                    ...
                    loop {
                        match raw_rx.recv_timeout(std::time::Duration::from_secs(2)) {
                            ...
                            Err(_) => {
                                log::warn!(
                                    "whispr-audio: timed out waiting for end-of-stream"
                                );
                                break;
                            }
                        }
