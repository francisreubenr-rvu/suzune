...
pub fn inject_auto_with_primary(
    ...
    let (fallback, primary_call, fallback_call): (
        ...
        InjectionMethod::DirectType => {
            ...
            log::info!("whispr-inject: injected via {}", InjectionMethod::DirectType);
    ...
    match primary_call(text) {
        Ok(()) => {
            log::info!("whispr-inject: injected via {primary}");
            Ok(primary)
        }
        Err(primary_err) => {
            log::info!(
                "whispr-inject: {primary} unavailable ({primary_err}), falling back to {fallback}"
            );
            fallback_call(text)?;
            log::info!("whispr-inject: injected via {fallback}");
            Ok(fallback)
        }
    }
}
